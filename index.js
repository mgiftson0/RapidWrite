...
function loadPara() {
    ...
    typingP.innerHTML = paras[randIndex].split("").map(char => `<span>${char}</span>`).join("");
...
function initTyping() {
    ...
    let typedChar = inputF.value.charAt(charIndex);
    ...
    if (charIndex < chars.length && timeLeft > 0) {
        ...
        // Move cursor and handle backspace
        if (typedChar == null) {
            if (charIndex > 0) {
                charIndex--;
...
function showScorePopup() {
    document.getElementById("wpmResult").innerText = `WPM: ${wpmT.innerText}`;
    document.getElementById("mistakeResult").innerText = `Mistakes: ${mistakeT.innerText}`;
    document.getElementById("cpmResult").innerText = `CPM: ${cpmT.innerText}`;
